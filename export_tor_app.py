# GhostCache — now with optional Tor hidden service support.
# If stem and a running Tor control socket are available, an ephemeral
# .onion address is created automatically.
...
try:
    ...
    # Define dummy exceptions/classes so the code still runs
...
def connect_to_tor():
    ...
    The service is stored in ``app.tor_service`` so it can be removed on exit.
    If stem is unavailable or Tor isn't running, the application continues
    in normal (surface web) mode.
    """
    if not TOR_AVAILABLE or Controller is None:
        print("stem library not available; skipping Tor connection.")
        return

    socket_path = "/var/run/tor/control"   # common on Tails/Linux
    try:
        ...
        with Controller.from_socket_file(path=socket_path) as controller:
            ...
            try:
                controller.authenticate()  # try no‑cookie method first
            ...
            # Create hidden service: map onion port 80 to local port 5000
            print("Creating hidden service...")
            service = controller.create_ephemeral_hidden_service(
                {80: 5000},                # remote port : local port
                await_publication=True      # wait for service to be published
            )
...
if __name__ == "__main__":
    ...
    # Attempt to create a Tor hidden service (if stem and Tor are available)
    connect_to_tor()

    try:
        # Run the Flask development server – note: host='0.0.0.0' makes it
        # reachable from the Tor service (which connects to localhost:5000)
        print("GhostCache starting on http://127.0.0.1:5000/")
        app.run(host='0.0.0.0', port=5000, debug=True)
    finally:
        # Clean up the hidden service when the server stops
